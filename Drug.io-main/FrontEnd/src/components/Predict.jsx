...
C1=CC=CC=C1 Benzene`
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      // Use the centralized API client which adds the Authorization header
      const resp = await api.post('/predict', {
        smiles: processedInput.map((i) => i.smiles)
      });
      ...
      // Improved error handling to capture backend validation errors
      const errorMessage = err.response?.data?.error || err.response?.data?.details || err.message || "Prediction failed. Check backend logs.";
